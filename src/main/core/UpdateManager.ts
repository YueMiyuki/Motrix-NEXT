...
export default class UpdateManager extends EventEmitter {
  ...
  setupUpdateFeed() {
    try {
      ...
      logger.error(`[Motrix] setup updater feed failed: ${msg}`)
    }
  }
  ...
  init() {
    ...
    if (this.autoCheckData.checkEnable && !this.isChecking && this.canCheckForUpdates()) {
      this.autoCheckData.userCheck = false
      this.updater.checkForUpdates().catch(() => {
        // error event is emitted by electron-updater; this prevents unhandled rejections.
      })
    }
  }
