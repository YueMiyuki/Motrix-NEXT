...
const actions = {
  ...
  fetchList ({ commit, state }) {
    return api.fetchTaskList({ type: state.currentList })
      ...
      .catch((err) => {
        console.warn('[Motrix] fetchList failed:', err.message)
  ...
  fetchItem ({ dispatch }, gid) {
    return api.fetchTaskItem({ gid })
      ...
      .catch((err) => {
        console.warn('[Motrix] fetchItem failed:', err.message)
        dispatch('updateCurrentTaskItem', null)
        return null
      })
  },
  fetchItemWithPeers ({ dispatch }, gid) {
    return api.fetchTaskItemWithPeers({ gid })
      ...
      .catch((err) => {
        console.warn('[Motrix] fetchItemWithPeers failed:', err.message)
        dispatch('updateCurrentTaskItem', null)
        return null
      })
  },
  showTaskDetailByGid ({ commit, dispatch }, gid) {
    return api.fetchTaskItem({ gid })
      ...
      .catch((err) => {
        console.warn('[Motrix] showTaskDetailByGid failed:', err.message)
        return null
      })
  },
  ...
  getTaskOption (_, gid) {
    return api.getOption({ gid })
      .catch((err) => {
        console.warn('[Motrix] getTaskOption failed:', err.message)
        return {}
      })
  },
