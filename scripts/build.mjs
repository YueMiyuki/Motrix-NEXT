...
function hasOption(rawArgs, flags) {
	return flags.some(
		(flag) =>
			...
			rawArgs.some((arg) => arg.startsWith(`${flag}=`)),
	);
}
...
const targetKey =
	resolvePlatformArchFromTarget(target) ?? resolvePlatformArchFromHost();
...
if (!userProvidedConfig && resourceConfig) {
	...
	console.log(
		chalk.yellow(
			"  warning: unknown target/host architecture, using default tauri resources",
		),
	);
}
